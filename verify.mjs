...
const before = await page.locator(".product-card:not(.is-hidden)").count();
await page.locator('[data-filter="qa"]').click();
await page.waitForTimeout(150);
const after = await page.locator(".product-card:not(.is-hidden)").count();

await page.locator('[data-add-quote="ACR Accredited Medium MRI Phantom"]').click();
await page.waitForTimeout(150);
const quoteOpen = await page.locator("[data-quote-drawer]").evaluate((el) => el.classList.contains("is-open"));
const quoteCount = await page.locator("[data-quote-count]").innerText();
...
console.log(
  JSON.stringify(
    {
      title,
      productCardsBeforeFilter: before,
      productCardsAfterQaFilter: after,
      quoteOpen,
      quoteCount,
    },
    null,
